...
export default function Register(){
  ...
  const onSubmit = async (e)=>{ e.preventDefault(); setErr(''); try{ const { data } = await api.post('/api/auth/register',{ name, email, password }); login(data.user, data.token); navigate('/'); }catch(e){ setErr(e?.response?.data?.error || e.message); } };
  return (
